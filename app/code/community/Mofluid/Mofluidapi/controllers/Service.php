<?php
/*
  Mofluidapi_Catalog_Products v0.0.1
    ...
    public function ws_category($store, $service) {
        ...
        try {
		    ...
		    $categories = $cat->getCollection()
			   	              ->addAttributeToSelect(array('name', 'image', 'description','store'))
	    ...
	    //if($cache->load($cache_key))
          ...
          foreach ($children as $category) {
			  $res[] = array(
                               ...
                               "image" => Mage::getModel('catalog/category')->load($category->getId())->getImageUrl(),
                               );
          }
...
->addAttributeToFilter('is_in_stock', array('in' => array($is_in_stock_option, 1)))
  ...
  //if(!$show_out_of_stock)
    ...
    // $defaultprice = str_replace(",","",number_format($_product->getFinalPrice(),2)); 
    ...
    public function ws_getFeaturedProducts($currentcurrencycode, $service, $store){
	  ...
	  $_products = Mage::getModel('catalog/product')->getCollection()
          ->joinField(
                        ...
                        '{{table}}.stock_id=1',
                        'left'
                )->addStoreFilter($store_id)->setOrder('created_at', 'desc');
...
// Get the Special Price
    ...
    public function ws_productinfo($store_id, $service, $productid, $currentcurrencycode) {
         $product = new Mofluidapi_Products($store_id, $service, $productid, $currentcurrencycode);
