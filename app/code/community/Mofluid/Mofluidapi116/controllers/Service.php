<?php
/*
  Mofluidapi116_Catalog_Products v0.0.1
    ...
    **/
    ...
    public function ws_category($store, $service) {
        ...
        try {
		    ...
		    $categories = $cat->getCollection()
			   	              ->addAttributeToSelect(array('name', 'thumbnail','image', 'description','store'))
    ...
    **/
    ...
    public function ws_products($store_id, $service, $categoryid,$curr_page,$page_size,$sortType,$sortOrder, $currentcurrencycode) {
	 ...
	 /*   if($cache->load($cache_key)) {
	  ...
	  $category->load($c_id);
	  $children1 = $category->getProductCollection()
->joinField(
                        'is_in_stock',
...
->addAttributeToFilter('is_in_stock', array('in' => array($is_in_stock_option, 1)))
 ...
 $res["total"] = count($children1);  
	  $collection = $category->getProductCollection()->joinField(
                        'is_in_stock',
                        'cataloginventory/stock_item',
                        'is_in_stock',
                        'product_id=entity_id',
                        '{{table}}.stock_id=1',
                        'left'
                )->addStoreFilter($store_id)->addAttributeToFilter('type_id', array('in' => array(Mage_Catalog_Model_Product_Type::TYPE_SIMPLE, Mage_Catalog_Model_Product_Type::TYPE_CONFIGURABLE, Mage_Catalog_Model_Product_Type::TYPE_GROUPED)));
...
//echo "<pre>";print_r("type ".$tax_type);  
//echo "<pre>";print_r("tax ".$tax_price);      
//echo "<pre>";print_r("price ".$defaultprice); 
   ...
   $res["data"][] = array(
                               ...
                               "price" => strval(round($this->convert_currency($defaultprice,$basecurrencycode,$currentcurrencycode),2)),
...
public function convert_currency($price,$from,$to) {
    ...
    public function ws_getFeaturedProducts($currentcurrencycode, $service, $store){
	  ...
	  $_products->addFieldToFilter(array(array('attribute' => 'featured', 'eq' => true),
      ));
     $_products->addAttributeToFilter('type_id', array('in' => array(Mage_Catalog_Model_Product_Type::TYPE_SIMPLE, Mage_Catalog_Model_Product_Type::TYPE_CONFIGURABLE)))->addAttributeToFilter('is_in_stock', array('in' => array($is_in_stock_option, 1))); 
...
// Get the Special Price
		...
		$featuredProducts["status"][0]=array('Show_Status'=>"1");
	  }
	  else
		$featuredProducts["status"][0]=array('Show_Status'=>"0");
...
// Get the Special Price
    ...
    public function ws_productinfo($store_id, $service, $productid, $currentcurrencycode) {
         $product = new Mofluidapi116_Products($store_id, $service, $productid, $currentcurrencycode);
       ...
       // if($cache->load($cache_key))
        ...
        try
        {
				 ...
				 if($cache_data['mofluid_cs_accountid']=='')
				 {
					...
					$cache_array=array('status'=>$cache_data['mofluid_cs_status'], 'cache_time'=>15);
				 }
				 else
				 {
					$cache_array=array('status'=>$cache_data['mofluid_cs_status'], 'cache_time'=>$cache_data['mofluid_cs_accountid']);
				 }
      	 }
	               	 		...
	               	 		if($mofluid_image_action->base == 'product') {
	               	        		$_products = Mage::getModel('catalog/product')->getCollection()
          										  	->joinField(
                        										...
                        										'{{table}}.stock_id=1',
                       										'left'
                										)->addStoreFilter($store)->addAttributeToFilter('entity_id', $mofluid_image_action->id);
	...
	public function ws_search($store, $service, $search_data, $curr_page, $page_size, $sortType, $sortOrder,  $currentcurrencycode) {
		...
		try {
		    ...
		    $total_product_collection = Mage::getResourceModel('catalog/product_collection')
			   ->joinField(
				  'is_in_stock',
				  'cataloginventory/stock_item',
				  'is_in_stock',
				  ...
				  'left')
			    ->addAttributeToSelect('*')
			    ->addAttributeToFilter('name', $search_condition)
    ...
    function mofluid_register_push($store, $deviceid, $pushtoken, $platform, $appname, $description) {
    		...
    		try {
               ...
               if($row["device_id"]) {
                        ...
                        $res= array ("status" => "update","deviceid" => $deviceid,"pushtoken" => $pushtoken,"message" =>"Update token for the existing device id.");
                }
...
$mofluid_push->query("DELETE FROM  ".$tab_prefix."mofluidpush WHERE push_token_id = '".$pushtoken."' AND app_name = '".$appname."' AND platform ='".$platform."'");
                        $mofluid_push->query("insert into ".$tab_prefix."mofluidpush (mofluidadmin_id, device_id, push_token_id, platform, app_name, description) 
                                                                      values (1,'".$deviceid."','".$pushtoken."','".$platform."','".$appname."','".$description."')");
                        $res= array ("status" => "update","deviceid" => $deviceid,"pushtoken" => $pushtoken,"message" =>"Update Device for the existing token id.");

}
                else {
                        $mofluid_push->query("insert into ".$tab_prefix."mofluidpush (mofluidadmin_id, device_id, push_token_id, platform, app_name, description) 
                                                                      values (1,'".$deviceid."','".$pushtoken."','".$platform."','".$appname."','".$description."')");
                         $res= array ("status" => "register","deviceid" => $deviceid,"pushtoken" => $pushtoken,"message" =>"register device id with new token.");
                }
          }
          catch(Exception $ex) {
                $res= array ("status" => "error","deviceid" => $deviceid,"pushtoken" => $pushtoken,"message" =>$ex->getMessage());
          }
          return $res;
    }
		...
		foreach($methods as $_ccode => $_carrier) {
			if($_methods = $_carrier->getAllowedMethods())  {
				...
				foreach($_methods as $_mcode => $_method)   {
					...
					$shipping[$_code]=array('method' => $_method,'title' => $_title);
				}
			}
		}
		 $cache->save(json_encode($shipping), $cache_key, array("mofluid"), $this->CACHE_EXPIRY); 
		return $shipping; 
	}
	...
	public function prepareQuote($custid, $Jproduct, $store, $address, $shipping_code, $couponCode, $currency, $is_create_quote,  $find_shipping){
	    ...
	    try {
			...
			Mage::app()->setCurrentStore($store);/*Added by Prafull to assign store to Mage*/
    ...
    public function setQuotePayment($quote, $pmethod, $transid) {
    	...
    	$quotePayment->setMethod($pmethod) 
    				 ->setIsTransactionClosed(1)
			      ->setTransactionAdditionalInfo(Mage_Sales_Model_Order_Payment_Transaction::RAW_DETAILS,array('TransactionID'=>$transid,'key2'=>'value2'));
	...
	public function placeorder($custid, $Jproduct, $store, $address, $couponCode, $is_create_quote, $transid, $payment_code, $shipping_code, $currency, $message) {
		...
		try {
			...
			$order->addStatusHistoryComment("Order was placed using Mobile App")
...
->addAttributeToFilter('is_in_stock', array('in' => array($is_in_stock_option, 1)))->addAttributeToFilter('visibility', 4)
                            ->load();            
              foreach ($collection as $_product) {                  $stock_status = 1;
                  $is_in_stock = $_product->getStockItem()->getIsInStock();
                  $stock_quantity = Mage::getModel('cataloginventory/stock_item')->loadByProduct($_product->getId())->getQty();
    ...
    public function ws_getmofluidextension() {
        ...
        $selectresource = $connection->select()
                        ->from(Mage::getSingleton('core/resource')
                        ->getTableName('mofluidadmin/mofluidresource'), array('*'));
   ...
   public function ws_getpaymentmethod() {
       ...
       $mofluid_pay_selectresource = $mofluid_pay_connection->select()
           ->from(Mage::getSingleton('core/resource')
           ->getTableName('mofluid_paymentcod/payment'), array('*'));
    ...
    foreach($parent_attVal as $parent_optionKey => $parent_optionVal)
     {
         ...
         $parent_inc++;
			...
			function ws_get_configurable_option_attributes($selectedValue, $label, $productid, $currentcurrencycode) {
			$cache = Mage::app()->getCache();
	 ...
	 $attributeOptions[$productAttribute['label']][$attribute['value_index']]["use_default_value"] = $attribute['use_default_value'];
				...
				$cache->save(json_encode($attributeOptions[$label][$selectedValue]), $cache_key, array("mofluid"), $this->CACHE_EXPIRY); 
