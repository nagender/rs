...
class Mofluid_Thememofluidelegant_Block_Adminhtml_Form_Edit_Tab_Themecolorforeground extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $form->setFieldNameSuffix('mofluidtheme_foreground');
        
        $mofluid_theme_elegant_model = Mage::getModel('mofluid_thememofluidelegant/colors');
        $mofluid_theme_elegant = $mofluid_theme_elegant_model->getCollection()->addFieldToFilter('mofluid_theme_id','1')->addFieldToFilter('mofluid_color_type','foreground');
        $mofluid_theme_elegant_data = $mofluid_theme_elegant->getData(); 
        
        //creating forground fieldset tab
        $foreground_fieldset = $form->addFieldset('foreground', array(
            'legend'       => $helper->__('Foreground'),
            'class'        => 'fieldset-wide'
        ));
         foreach($mofluid_theme_elegant_data as $key=>$value) {
            ...
            $foreground_fieldset->addField($field_id, 'text', array(
                'name'  => $field_id,
